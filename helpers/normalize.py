    ...
    for period_key, shops in data.items():
        ...
        for shop_id_str, shop_info in shops.items():
            ...
            if not isinstance(dates_dict, dict) or not dates_dict:
                st.write("DEBUG: dates_dict leeg of geen dict → skip shop")
                continue
            ...
            for date_label, date_entry in dates_dict.items():
                ...
                if isinstance(date_entry, dict):
                    ...
                    day_data = {}
                    st.write("DEBUG: date_entry geen dict → gebruik leeg")

                st.write(f"DEBUG: day_data keys = {list(day_data.keys())}")

                dt_raw = day_data.get("dt")
                st.write(f"DEBUG: dt_raw = {dt_raw} (type: {type(dt_raw)})")

                # VERWIJDER continue VOOR dt_raw
                # if not dt_raw:
                #     continue

                # GEBRUIK label als fallback
                if dt_raw:
                try:
                ...
                def safe_float(val, default=0.0):
                    try:
                        ...
                        st.write(f"DEBUG: float fout: {val} → {e}")
                        return default
                ...
                def safe_int(val, default=0):
                    try:
                        ...
                        st.write(f"DEBUG: int fout: {val} → {e}")
                        return default
