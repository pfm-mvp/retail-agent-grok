# pages/retailgift_regio.py – 100% WERKENDE REGIO MANAGER MET LIVE CBS + PERIODE + % VERGELIJKING (25 nov 2025)
...
period_option = st.sidebar.selectbox("Periode", ["this_month", "last_month", "this_year", "last_year"], index=0)
...
first_of_last_year = first_of_year - pd.DateOffset(years=1)

# --- FILTER OP PERIODE ---
if period_option == "this_month":
    df_period = df[(df["date"] >= first_of_month)]
    prev_period = df[(df["date"] >= first_of_last_month) & (df["date"] < first_of_month)]
elif period_option == "last_month":
    df_period = df[(df["date"] >= first_of_last_month) & (df["date"] < first_of_month)]
    prev_period = df[(df["date"] >= first_of_last_month - pd.DateOffset(months=1)) & (df["date"] < first_of_last_month)]
elif period_option == "this_year":
    df_period = df[df["date"] >= first_of_year]
    prev_period = df[(df["date"] >= first_of_last_year) & (df["date"] < first_of_year)]
elif period_option == "last_year":
    df_period = df[(df["date"] >= first_of_last_year) & (df["date"] < first_of_year)]
    prev_period = df[(df["date"] >= first_of_last_year - pd.DateOffset(years=1)) & (df["date"] < first_of_last_year)]

# --- KPI'S MET % VERGELIJKING ---
current = df_period.agg({"count_in": "sum", "conversion_rate": "mean", "turnover": "sum"})
prev = prev_period.agg({"count_in": "sum", "conversion_rate": "mean", "turnover": "sum"}) if not prev_period.empty else current.copy()
...
c1.metric("Totaal Footfall", f"{int(current['count_in']):,}", delta(current['count_in'], prev['count_in']))
c2.metric("Gem. Conversie", f"{current['conversion_rate']:.1f}%", delta(current['conversion_rate'], prev['conversion_rate']))
c3.metric("Totaal Omzet", f"€{int(current['turnover']):,}", delta(current['turnover'], prev['turnover']))

# --- LIVE CBS DATA + GRAFIEK ---
