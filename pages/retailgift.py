# pages/retailgift.py – FINAL & THIS_WEEK WERKT
...
# --- 6. AGGREGEER VOOR WEEK/MONTH ---
if period in ["this_week", "last_week", "this_month", "last_month"]:
    agg = {
...
if role == "Store Manager" and len(selected) == 1:
    ...
    agg = df[["count_in", "conversion_rate", "turnover"]].sum() if "TOTAAL" in df["name"].values else df.agg({"count_in": "sum", "conversion_rate": "mean", "turnover": "sum"})
...
st.caption("RetailGift AI: `period_step=day` + aggregatie = perfecte weekdata.")
