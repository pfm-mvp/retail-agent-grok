...
if period_option == "yesterday":
    ...
    df_raw = df_raw[(df_raw["date"] >= start_week) & (df_raw["date"] <= end_week)]
    ...
    df_raw = df_raw[(df_raw["date"] >= first_of_month) & (df_raw["date"] <= last_of_this_month)]
elif period_option == "last_month":
    df_raw = df_raw[(df_raw["date"] >= first_of_last_month) & (df_raw["date"] < first_of_month)]
...
if tool == "Store Manager" and len(selected) == 1:
    ...
    # === DEZE FUNCTIE IS VOLLEDIG VERVANGEN – NOOIT MEER N/A ===
    ...
    st.header(f"{row['name']} – {period_option.replace('_', ' ').capitalize()}")
    ...
    # De rest van je Store Manager tool blijft 100% ongewijzigd
    # (dagelijkse tabel, voorspelling, grafiek, actie – allemaal perfect)

    # --- DAGELIJKSE TABEL, VOORSPELLING, GRAFIEK, ACTIE ---
    # (jouw originele code blijft hier exact staan – kopieer vanaf jouw versie)
    # Ik laat het hier even weg voor overzicht, maar je plakt gewoon jouw bestaande blokken terug
    ...
    # --- VOORSPELLING: REALISTISCHE OMZET ---
